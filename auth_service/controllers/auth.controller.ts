...
export const register = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (user_role === "doctor") {
      ...
      if (govt_id) {
        newUserData.govt_id = govt_id;
      }
      if (req.body.hospital_id) {
      newUserData.hospital_id = req.body.hospital_id;
  }
    }


    if (user_role === "doctor" && organisation_id) {
    ...
    if (user_role === "hospital" && organisation_id) {
      newUserData.organisation_id = organisation_id;
    }
...
export const updateUser = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    user.approved = approved || user.approved; // Only update if provided
    await user.save();
...
export const getHospitals = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    let users = await User.find({ user_role: "hospital" });
...
export const getLabs = async (req: Request, res: Response) => {
  try {
    const labs = await User.find({ user_role: 'lab' });
    res.status(200).json({ message: 'Labs fetched successfully', users: labs });
  } catch (err) {
